...
export default function TrashPage() {
  ...
  const fetchTrash = async () => {
    try {
      setLoading(true);
      const data = await emailApi.getTrash();
      setEmails(data);
    } catch (error: any) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {emails.length} deleted {emails.length === 1 ? "email" : "emails"}
                </p>
              </div>
            </div>
            <Button variant="outline" size="icon" onClick={fetchTrash} disabled={loading}>
          ...
          <div className="lg:col-span-1">
            <Card className="p-4">
              <nav className="space-y-2">
                ...
                <Button variant="ghost" className="w-full justify-start" onClick={() => router.push("/archive")}>
